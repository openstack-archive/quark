...
function run_tests {
  ...
  OS_TEST_PATH=`echo $testargs|grep -o 'quark\.tests[^[:space:]:]*\+'|tr . /`
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='quark/*' -d covhtml -i
  fi
...
function run_pep8 {
  ...
  ${wrapper} flake8 --builtins=_ --ignore=H302 quark/
}
