...
class QuarkSGAsyncProcessCallback(object):
    ...
    def update_sg(self, context, sg):
        db_sg = db_api.security_group_find(context, id=sg, scope=db_api.ONE)
        if not db_sg:
            return None
        with context.session.begin():
            job_body = dict(action="update sg %s" % db_sg.id,
...
class QuarkSGAsyncProcessClient(object):
    ...
    def start_update(self, context, sg):
        cctxt = self.client.prepare(version='1.0')
        return cctxt.call(context, 'update_sg', sg=sg)
...
class QuarkSGProducerCallback(object):
    ...
    def populate_subtasks(self, context, sg, parent_job_id):
        ...
        if not db_sg:
            return None
        ports = _gather_associated_ports(context, db_sg)
        if len(ports) == 0:
            return {"ports": 0}
        for port in ports:
...
def main():
    test_client_ops = [
        cfg.StrOpt('sg_id', default=None, help=_("The SG to test")),
    ]
    CONF.register_cli_opts(test_client_ops)
    ...
    sg_id = CONF.sg_id
    if sg_id is None:
        sys.exit(_("Usage:\n%s --sg_id=<uuid>" % sys.argv[0]))

    consumer_client = QuarkSGAsyncClient(CONSUMER_TOPIC)
    res = consumer_client.consume(base_worker.get_test_context(), sg_id)
    LOG.debug(res)
    time.sleep(0)
